...
export function DataTable<TData, TValue>({ columns, data }: DataTableProps<TData, TValue>) {
	...
	const [filterStateValue, setFilterStateValue] = useState("status");
	...
	return (
		<div>
			<div className="flex items-center gap-4">
				...
				<div className="flex items-center gap-2 py-4">
					<Select
						value={(table.getColumn(filterStateValue)?.getFilterValue() as string) ?? ""}
						onValueChange={(value) => {
							// Clear filter if "all" or empty, otherwise set the selected value
							if (value === "all" || value === "") {
								table.getColumn(filterStateValue)?.setFilterValue("");
							} else {
								table.getColumn(filterStateValue)?.setFilterValue(value);
							}
						}}>
						<SelectTrigger className="w-max">
